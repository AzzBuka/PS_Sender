...
3. Скрипт исполняется с ключами командной строки:
 ...
 5. Иногда случается так, что ПО вырезает кавычки, которые необходимы в командной строке. В этом случае нужно использовать хелпер для командной
    строки PS_Sender.bat "You text message" (Или переменные %name %status %lastvalue). Соответственно в файле хелпера надо вписать свои значения.
